...
def main(folder: str, filename: str) -> None:
    ...
    response = httpx.post("http://192.168.0.101:8000/session/start", json={"filename": filename})
    ...
    response = httpx.delete(f"http://192.168.0.101:8000/session/{task_id}")
    if response.status_code != 200:
        return

    response = httpx.get(f"http://192.168.0.101:8000/static/{filename}")
    ...
    sleep(2)
    response = httpx.delete(f"http://192.168.0.101:8000/file/{filename}")
    if response.status_code != 200:
        return
